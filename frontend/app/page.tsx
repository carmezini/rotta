...
export default function Home() {
  ...
  const [filterType, setFilterType] = useState<
    "all" | "limit" | "streak" | "accumulate"
  >("all");

  // Modal state
  const [activeModal, setActiveModal] = useState<
    "create" | "edit" | "checkin" | null
  >(null);
  ...
  const [type, setType] = useState<"limit" | "streak" | "accumulate">(
    "accumulate",
  );
  ...
  const [loadingLogsGoalId, setLoadingLogsGoalId] = useState<string | null>(
    null,
  );
  ...
  const fetchGoals = async () => {
    const res = await fetch(`${API_URL}/goals?user_id=${USER_ID}`);
    if (!res.ok) {
  ...
  const fetchCheckIns = async (goalId: string) => {
    try {
      ...
      const res = await fetch(`${API_URL}/goals/${goalId}/checkins`);
      if (res.ok) {
  ...
  const handleUpdateGoal = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch(`${API_URL}/goals/${selectedGoal.id}`, {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          name,
  ...
  const handleAddCheckIn = async (e: React.FormEvent) => {
    ...
    if (!selectedGoal) return;
    try {
      const res = await fetch(`${API_URL}/goals/${selectedGoal.id}/checkin`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
  ...
  const filteredGoals = goals.filter(
    (g) => filterType === "all" || g.type === filterType,
  );
    ...
    <div className="app-shell sm:p-6 font-sans flex items-start justify-center">
      <main className="app-frame w-full max-w-5xl bg-white border-4 border-black rounded-[28px] flex flex-col min-h-[calc(100dvh-1rem)] sm:min-h-[calc(100dvh-3rem)] shadow-[10px_10px_0_#111] overflow-hidden">
          ...
          <div className="mt-4 bg-black text-white p-3 border-2 border-white rounded-xl flex justify-between items-center shadow-[4px_4px_0px_0px_rgba(255,255,255,1)]">
            ...
            <div className="text-center flex-1">
              <span className="block text-2xl font-black text-primary-blue">
                {
                  goals.filter(
                    (g) => g.type === "streak" && g.current_value > 0,
                  ).length
                }
              </span>
              <span className="text-[10px] uppercase font-bold text-zinc-400">
        ...
        <div className="flex-1 p-4 overflow-y-auto grid grid-cols-1 md:grid-cols-2 gap-5 bg-zinc-50 content-start">
          ...
          {error && (
            <div className="col-span-full p-4 bg-primary-red border-3 border-black rounded-xl shadow-[4px_4px_0px_0px_rgba(0,0,0,1)]">
          ...
          {!loading && !error && filteredGoals.length === 0 && (
            <div className="col-span-full py-12 px-4 text-center border-4 border-dashed border-zinc-400 rounded-2xl bg-white">
              <p className="text-lg font-black text-zinc-400 mb-2">
                Nenhuma meta por aqui! 📭
              </p>
            ...
            filteredGoals.map((goal) => {
              ...
              return (
                ...
                >
                  ...
                  <div
                    className={`p-4 border-b-3 border-black flex justify-between items-start gap-2 ${getGoalTypeColor(goal.type)}`}
                  >
                    <div>
                      ...
                      <h2 className="text-xl font-black text-black tracking-tight line-clamp-1">
                        {goal.name}
                      </h2>
                    </div>
                  ...
                  <div className="p-4 flex-1 flex flex-col gap-3">
                    ...
                    <div className="flex gap-2 mt-2 pt-2 border-t border-zinc-200">
                      ...
                      <button
                        ...
                        className="bg-white text-black font-bold border-2 border-black p-2 rounded-lg text-xs shadow-[2px_2px_0px_0px_rgba(0,0,0,1)] active:translate-x-0.5 active:translate-y-0.5 active:shadow-none hover:bg-zinc-100 transition-all flex items-center justify-center"
                        title="Histórico"
                      >
                        <span>📜</span>
                      </button>
                      ...
                      <button
                        ...
                        className="bg-primary-red text-white font-bold border-2 border-black p-2 rounded-lg text-xs shadow-[2px_2px_0px_0px_rgba(0,0,0,1)] active:translate-x-0.5 active:translate-y-0.5 active:shadow-none hover:bg-red-500 transition-all flex items-center justify-center"
                        title="Excluir"
                      >
                        <span>🗑️</span>
                      </button>
                  ...
                  {expandedGoalId === goal.id && (
                    <div className="bg-zinc-100 border-t-2 border-black p-3 space-y-2 text-xs">
                      ...
                      {checkInLogs[goal.id] &&
                        checkInLogs[goal.id].length === 0 && (
                          <p className="text-zinc-500 py-2 text-center font-bold">
            ...
            <div className="p-5 overflow-y-auto overscroll-contain">
              ...
              {(activeModal === "create" || activeModal === "edit") && (
                ...
                >
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    <div>
                      ...
                      <select
                        ...
                        onChange={(e) =>
                          setType(e.target.value as Goal["type"])
                        }
                    ...
                    <div>
                      ...
                      <select
                        value={period}
                        onChange={(e) =>
                          setPeriod(e.target.value as Goal["period"])
                        }
                        className="w-full bg-white border-2 border-black rounded-lg p-2.5 text-sm font-bold text-black outline-none focus:ring-2 focus:ring-black"
                      >
                  ...
                  <button
                    type="submit"
                    className="w-full bg-primary-pink text-black font-black border-3 border-black p-3 rounded-lg text-sm shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] active:translate-x-1 active:translate-y-1 active:shadow-none transition-all mt-6 uppercase tracking-wider"
                  >
                    {activeModal === "create"
                      ? "Criar Meta 🏁"
                      : "Salvar Alterações 💾"}
                ...
                <form onSubmit={handleAddCheckIn} className="space-y-4">
                  <div className="bg-zinc-100 border-2 border-black p-3.5 rounded-xl mb-4">
                    ...
                    <p className="text-xs text-zinc-600 font-semibold mt-1">
                      Progresso Atual: {selectedGoal.current_value} /{" "}
                  ...
                  <button
                    type="submit"
                    className="w-full bg-primary-green text-black font-black border-3 border-black p-3 rounded-lg text-sm shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] active:translate-x-1 active:translate-y-1 active:shadow-none transition-all mt-6 uppercase tracking-wider"
                  >
                    Registrar Check-In 🚀
                  </button>
                </form>
